...
Multiple BibTex files can be parsed using `lapply()`. The paths to the BibTex files must be stored in a vector. Using this vector to call `bib2df()` within `lapply()` results in a list of `data.frames`, which can be bound, e.g. using `bind_rows()`:
